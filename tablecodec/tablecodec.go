...
// normal key consists of tablePrefix + tableID + recordPrefixSep + handle
// total length is RecordRowKeyLen
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	k := key
	tableID, _, isRecord, err := DecodeKeyHead(key)
	if err != nil {
		return 0, 0, errors.Trace(err)
	}
	if !isRecord {
		return 0, 0, errInvalidIndexKey.GenWithStack("invalid index key - %q", k)
	}

	// obtain handle using codec.DecodeInt
	key = key[prefixLen:]
...
// normal index_key consists of tablePrefix + tableID + indexPrefixSep + idxID + encodedValue
// total length is prefixLen + len(encodedValue)
